...
The formats above are illustrative human-readable shapes, not a
requirement for sequential canonical ID allocation. Canonical V0 IDs
keep these prefixes (`FW-` / `DOC-` / `CL-` / `TOPIC-` / `MAP-`) but
derive their suffix deterministically from stable identity-defining
fields, independent of database row order (see ADR-018 and
`backend/models/ids.py`). Sequential or display-oriented IDs may be
added later at the persistence layer without replacing the canonical
domain IDs.
